...
#This version of testing will only include Arsenal and Aston Villa in the game simulations
teams_in_league_temporary = ['Arsenal', 'Aston Villa']

#This loop will load Arsenal and Aston Villa into the "current_data" dictionary 
for team in teams_in_league_temporary :
    if pi :
...
while manager_loop :
    ...
    while standings :
        display.fill(white)
        display.blit(standings_text, (0, 0))
        for event in pygame.event.get() :
            if event.type == pygame.KEYDOWN :
                if event.key == pygame.K_RIGHT :
                    standings = False
